...
// build/dev/javascript/gaussian/matrix.mjs
class RowOutOfBounds extends CustomType {
}
class ZeroValueError extends CustomType {
}
class SameRowError extends CustomType {
}
function new$7(rows, cols) {
  return repeat(repeat(0, cols), rows);
}
function from_lists(data2) {
  return data2;
}
...
function columns(matrix) {
  ...
  let _pipe$2 = map4(_pipe$1, length);
  return unwrap(_pipe$2, 0);
}
function get_cell(matrix, row, col) {
  let _pipe = matrix;
  let _pipe$1 = drop(_pipe, row);
  let _pipe$2 = first(_pipe$1);
  return try$(_pipe$2, (row_data) => {
    let _pipe$3 = row_data;
    let _pipe$4 = drop(_pipe$3, col);
    return first(_pipe$4);
  });
}
...
function set_row(matrix, row, row_data) {
  let _pipe = matrix;
  return index_map(_pipe, (r, idx) => {
    let $ = idx === row;
    if ($) {
      return row_data;
    } else {
...
function swap_rows(matrix, row1, row2) {
  let $ = row1 === row2;
  if ($) {
    return new Error(new SameRowError);
  } else {
    let row_count = rows(matrix);
    let $1 = row1 >= 0 && row1 < row_count && row2 >= 0 && row2 < row_count;
    if ($1) {
      return try$((() => {
        let _pipe = get_row(matrix, row1);
        return replace_error(_pipe, new RowOutOfBounds);
      })(), (r1) => {
        return try$((() => {
          let _pipe = get_row(matrix, row2);
          return replace_error(_pipe, new RowOutOfBounds);
        })(), (r2) => {
          let _pipe = matrix;
          let _pipe$1 = set_row(_pipe, row1, r2);
          let _pipe$2 = set_row(_pipe$1, row2, r1);
          return new Ok(_pipe$2);
        });
      });
    } else {
      return new Error(new RowOutOfBounds);
    }
  }
}
function eliminate_row(matrix, source_row, target_row, column) {
  ...
  if ($) {
    ...
    let $1 = source_row >= 0 && source_row < row_count && target_row >= 0 && target_row < row_count && column >= 0 && column < col_count;
    if ($1) {
      return try$((() => {
        let _pipe = get_cell(matrix, source_row, column);
        return replace_error(_pipe, new RowOutOfBounds);
      })(), (source_cell) => {
        return try$((() => {
          let _pipe = get_cell(matrix, target_row, column);
          return replace_error(_pipe, new RowOutOfBounds);
        })(), (target_cell) => {
          let $2 = source_cell === 0 || target_cell === 0;
          if ($2) {
            return new Error(new ZeroValueError);
          } else {
            let multiplier = 0 - divideFloat(target_cell, source_cell);
            return try$((() => {
              let _pipe = get_row(matrix, source_row);
              return replace_error(_pipe, new RowOutOfBounds);
            })(), (source_data) => {
              return try$((() => {
                let _pipe = get_row(matrix, target_row);
                return replace_error(_pipe, new RowOutOfBounds);
              })(), (target_data) => {
                let _block;
                let _pipe = zip(target_data, source_data);
                _block = map2(_pipe, (pair) => {
                  let target_val;
                  let source_val;
                  target_val = pair[0];
                  source_val = pair[1];
                  return target_val + multiplier * source_val;
                });
                let new_target_row = _block;
                let _pipe$1 = matrix;
                let _pipe$2 = set_row(_pipe$1, target_row, new_target_row);
                return new Ok(_pipe$2);
              });
            });
          }
...
// build/dev/javascript/gaussian/gaussian.mjs
var FILEPATH = "src/gaussian.gleam";

class Model extends CustomType {
  constructor(current_matrix, selected_row, history, input_mode) {
    super();
...
class HistoryEntry extends CustomType {
  constructor(matrix, description) {
    super();
    this.matrix = matrix;
    this.description = description;
  }
}
class SettingDimensions extends CustomType {
}
class EditingMatrix extends CustomType {
  constructor(rows2, cols, values3) {
    super();
    this.rows = rows2;
    this.cols = cols;
    this.values = values3;
...
class Operating extends CustomType {
}
class SetDimensions extends CustomType {
  constructor(rows2, cols) {
    super();
    this.rows = rows2;
    this.cols = cols;
  }
}
class UpdateCell extends CustomType {
  constructor(index4, value2) {
    super();
    this.index = index4;
    this.value = value2;
  }
}
class InitializeMatrix extends CustomType {
}
class SelectRow extends CustomType {
  constructor(row) {
    super();
    this.row = row;
  }
}
class PerformSwap extends CustomType {
  constructor(row) {
    super();
    this.row = row;
  }
}
class PerformElimination extends CustomType {
  constructor(row, col) {
    super();
    this.row = row;
    this.col = col;
...
class Undo extends CustomType {
}
class NoOp extends CustomType {
}
function init(_) {
  return [
    new Model(new$7(0, 0), new None, toList([]), new SettingDimensions),
    none()
  ];
}
function chunk_list(items, chunk_size) {
  if (items instanceof Empty) {
    return items;
  } else {
    let chunk = take(items, chunk_size);
    let rest = drop(items, chunk_size);
    return prepend(chunk, chunk_list(rest, chunk_size));
  }
}
function update2(model, msg) {
  if (msg instanceof SetDimensions) {
    let rows2 = msg.rows;
    let cols = msg.cols;
    let total = rows2 * cols;
    return [
      new Model(model.current_matrix, model.selected_row, model.history, new EditingMatrix(rows2, cols, repeat("0", total))),
      none()
    ];
  } else if (msg instanceof UpdateCell) {
    let index4 = msg.index;
    let value2 = msg.value;
    let $ = model.input_mode;
    if ($ instanceof EditingMatrix) {
      let rows2 = $.rows;
      let cols = $.cols;
      let values3 = $.values;
      let new_values = index_map(values3, (v, i) => {
        let $1 = i === index4;
        if ($1) {
          return value2;
        } else {
      ...
      return [
        new Model(model.current_matrix, model.selected_row, model.history, new EditingMatrix(rows2, cols, new_values)),
        none()
      ];
    } else {
      return [model, none()];
    }
  } else if (msg instanceof InitializeMatrix) {
    let $ = model.input_mode;
    if ($ instanceof EditingMatrix) {
      let rows2 = $.rows;
      let cols = $.cols;
      ...
      let float_values = _block;
      let matrix_data = chunk_list(float_values, cols);
      let new_matrix = from_lists(matrix_data);
      return [
        new Model(new_matrix, new None, toList([]), new Operating),
        none()
      ];
    } else {
      return [model, none()];
    }
  } else if (msg instanceof SelectRow) {
    let row = msg.row;
    let $ = model.selected_row;
    if ($ instanceof Some) {
      let r = $[0];
      if (r === row) {
        return [
          new Model(model.current_matrix, new None, model.history, model.input_mode),
          none()
        ];
      } else {
        return [
          new Model(model.current_matrix, new Some(row), model.history, model.input_mode),
          none()
        ];
      }
    } else {
  ...
  } else if (msg instanceof PerformSwap) {
    let target_row = msg.row;
    let $ = model.selected_row;
    if ($ instanceof Some) {
      let source_row = $[0];
      let $1 = swap_rows(model.current_matrix, source_row, target_row);
      if ($1 instanceof Ok) {
        let new_matrix = $1[0];
        let description = "Swapped R" + to_string(source_row) + " ↔ R" + to_string(target_row);
        return [
          new Model(new_matrix, new None, prepend(new HistoryEntry(model.current_matrix, description), model.history), model.input_mode),
          none()
        ];
      } else {
        return [model, none()];
  ...
  } else if (msg instanceof PerformElimination) {
    let target_row = msg.row;
    let col = msg.col;
    let $ = model.selected_row;
    if ($ instanceof Some) {
      let source_row = $[0];
      let $1 = eliminate_row(model.current_matrix, source_row, target_row, col);
      if ($1 instanceof Ok) {
        let new_matrix = $1[0];
        let description = "Eliminated R" + to_string(target_row) + "[" + to_string(col) + "] using R" + to_string(source_row);
        return [
          new Model(new_matrix, new None, prepend(new HistoryEntry(model.current_matrix, description), model.history), model.input_mode),
          none()
        ];
  ...
  } else if (msg instanceof Undo) {
    let $ = model.history;
    if ($ instanceof Empty) {
      return [model, none()];
    } else {
      let rest = $.tail;
      let prev_matrix = $.head.matrix;
      return [
        new Model(prev_matrix, new None, rest, model.input_mode),
        none()
      ];
    }
  } else {
...
function view_dimension_input() {
  return div(toList([]), toList([
    h2(toList([]), toList([text2("Enter Matrix Dimensions")])),
    p(toList([]), toList([text2("Rows:")])),
    input(toList([
      type_("number"),
      value("3"),
      id("rows-input")
    ])),
    p(toList([]), toList([text2("Columns:")])),
    input(toList([
      type_("number"),
      value("4"),
      id("cols-input")
    ])),
    button(toList([on_click(new SetDimensions(3, 4))]), toList([text2("Create Matrix")]))
  ]));
}
function view_matrix_input(rows2, cols, values3) {
  return div(toList([]), toList([
    h2(toList([]), toList([text2("Enter Matrix Values")])),
    div(toList([]), toList([
      table(toList([]), toList([
        tbody(toList([]), (() => {
          let _pipe = range(0, rows2 - 1);
          return map2(_pipe, (r) => {
            return tr(toList([]), (() => {
              let _pipe$1 = range(0, cols - 1);
              return map2(_pipe$1, (c) => {
                let index4 = r * cols + c;
                let _block;
                let $ = (() => {
                  let _pipe$2 = drop(values3, index4);
                  return first(_pipe$2);
                })();
                if ($ instanceof Ok) {
                  let v = $[0];
                  _block = v;
                ...
                let value2 = _block;
                return td(toList([]), toList([
                  input(toList([
                    type_("text"),
                    value(value2),
                    on_input((v) => {
                      return new UpdateCell(index4, v);
                    })
                  ]))
                ]));
              });
    ...
    button(toList([on_click(new InitializeMatrix)]), toList([text2("Start Operating")]))
  ]));
}
function handle_cell_click(model, row, col) {
  let $ = model.selected_row;
  if ($ instanceof Some) {
    let selected = $[0];
    if (selected !== row) {
      return new PerformElimination(row, col);
    } else {
      return new NoOp;
    }
  } else {
...
function view_matrix_display(model) {
  let row_count = rows(model.current_matrix);
  let col_count = columns(model.current_matrix);
  return div(toList([]), toList([
    h2(toList([]), toList([text2("Current Matrix")])),
    table(toList([class$("matrix-display")]), toList([
      tbody(toList([]), (() => {
        let _pipe = range(0, row_count - 1);
        return map2(_pipe, (r) => {
          let _block;
          let $ = model.selected_row;
          if ($ instanceof Some) {
          ...
          let _block$1;
          if (is_selected) {
            _block$1 = "selected-row";
          } else {
            _block$1 = "";
          }
          let row_class = _block$1;
          return tr(toList([
            class$(row_class),
            on_click(new SelectRow(r))
          ]), prepend(td(toList([class$("row-label")]), toList([text2("R" + to_string(r))])), (() => {
            let _pipe$1 = range(0, col_count - 1);
            return map2(_pipe$1, (c) => {
              let _block$2;
              let $1 = get_cell(model.current_matrix, r, c);
              if ($1 instanceof Ok) {
                let v = $1[0];
                _block$2 = float_to_string(v);
              } else {
                _block$2 = "?";
              }
              let value2 = _block$2;
              ...
              let $2 = c === col_count - 1;
              if ($2) {
                _block$3 = "augmented-column";
              } else {
                _block$3 = "";
              }
              let cell_class = _block$3;
              return td(toList([
                class$(cell_class),
                on_click(handle_cell_click(model, r, c))
              ]), toList([text2(value2)]));
            });
          })()));
        });
    ...
    div(toList([]), toList([
      button(toList([
        on_click(new Undo),
        disabled(is_empty2(model.history))
      ]), toList([text2("Undo")]))
    ]))
  ]));
}
function view_history(history) {
  return div(toList([]), toList([
    h3(toList([]), toList([text2("Operation History")])),
    ul(toList([]), (() => {
      let _pipe = history;
      let _pipe$1 = reverse(_pipe);
      return map2(_pipe$1, (entry) => {
        return li(toList([]), toList([text2(entry.description)]));
      });
    })())
  ]));
}
function view_operating_mode(model) {
  return div(toList([]), toList([view_matrix_display(model), view_history(model.history)]));
}
function view(model) {
  return div(toList([]), toList([
    h1(toList([]), toList([text2("Gaussian Elimination Tool")])),
    (() => {
      let $ = model.input_mode;
      if ($ instanceof SettingDimensions) {
        return view_dimension_input();
      } else if ($ instanceof EditingMatrix) {
        let rows2 = $.rows;
        let cols = $.cols;
        let values3 = $.values;
        return view_matrix_input(rows2, cols, values3);
      } else {
        return view_operating_mode(model);
      }
    })()
  ]));
}
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "gaussian", 15, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 341, end: 390, pattern_start: 352, pattern_end: 357 });
  }
  return;
}

// .lustre/build/gaussian.mjs
main();
